...
class PreviousBonusReviewController extends Controller
{
    //
    function salaryReview(User $user){
        ...
        if($author != $sbu_name && $author != $pm_name && Auth::user()->role != 'Admin'){
            ...
            return redirect()->route('employee-bonus-reviews-calculation.index')->with('warning', 'This user is unauthorized to you');
        }
        ...
        return view('previous-reviews.previous-bonus-only-review', 
            [
                'employeeID' => $user,
    ...
    function previousSalaryReviewUpdate(User $user, Request $request){
        ...
        $salary_rivews = SalaryReview::select('id')->orderBy('start', 'desc')->get();
        $previous_review_id = $salary_rivews[0]->id;
        ...
        return redirect()->route('employee-bonus-reviews-calculation.index')->with('flash', 'Updated previous review successfully');
